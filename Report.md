...
便携式环境监测系统分为两个部分，第一个部分是一个传感器模块，它分为三个主要部分：1. 传感器组，包括PM2.5传感器、紫外传感器、甲醛传感器、温湿度传感器；2. 电源，包括充电电路和稳压电路；蓝牙模块。这三个部分最后由有一个控制器来进行整体控制。

![](./pic/系统框图.jpg) 

第二个部分是手机上的数据接收和报警功能部分，通过蓝牙让传感器模块和手机可以实时通讯。
